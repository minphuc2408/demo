            ...
            for (int i = 0; i < enemyX.size(); ++i) {
                if (enemyFlag[i]) {
                    ...
                    if (enemyY[i] > SCREEN_HEIGHT) {
                        ...
                        enemyX[i] = 100 + (i % 3) * 200 + rand() % 100; // Random position within each lane
...
bool loadTextures() {
    SDL_Surface* surface = IMG_Load("./car.png"); // Path to car image
    if (!surface) {
    ...
    surface = IMG_Load("track.png"); // Path to track image
    if (!surface) {
...
void createEnemies(int numEnemies) {
    for (int i = 0; i < numEnemies; ++i) {
        enemyX.push_back(100 + (i % 3) * 200 + rand() % 100); // Random position within each lane
...
void drawWaitingScreen() {
    SDL_Color white = {255, 255, 255};
    std::string message = "Press O to Start";
